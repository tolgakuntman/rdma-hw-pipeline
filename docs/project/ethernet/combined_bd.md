...
We tested 16 KB of data (4 KB of 32-bit words) without any loss between the KR260 TX and KR260 RX boards. Everything works correctly. We could have handled the little-endian byte-reversal issue inside our custom Ethernet IPs, but since our final design uses two KR260 boards communicating with each other over Ethernet for RDMA, we decided not to modify anything—the two sides effectively cancel out the endianness reversal. We only see this effect for python script tests.
