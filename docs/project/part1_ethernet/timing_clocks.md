...
This chapter explains how clocks and resets are organized in **Part 1 – PL Ethernet** on the KR260, and why the exact frequencies (25 → 125 → 200 → 100 MHz) matter for the **AXI 1G/2.5G Ethernet Subsystem, RGMII I/O, and our custom RX/TX logic**.

We focus on:

- Which clock each IP core uses.
- How we generate 125 MHz and 200 MHz from the 25 MHz board clock.
...
> **NOTE (screenshot):** Add a block-diagram screenshot of the Vivado design with the **Clocking Wizard** and three `proc_sys_reset` blocks highlighted.
...
1. **25 MHz PL reference clock**  
   - Comes from the SOM connector into the PL.  
...
2. **100 MHz PS → PL clock (`pl_clk0`)**  
   - Generated by the Zynq UltraScale+ MPSoC PS.  
   - Exported into the PL through the **PS block** (`zynq_ultra_ps_e`).  
   - Used as the **AXI-Lite and “system” clock** for control/status paths.

> **NOTE (screenshot):** Add a screenshot of the PS configuration GUI showing `pl_clk0 = 100 MHz`.

All other clocks in the PL are derived from the 25 MHz input via the Clocking Wizard.
